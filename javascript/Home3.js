let apiPartyApi = new TempApi.PartyApi();import TempApi from '../src/index';document.getElementById('izt7hv').onclick = (event) => {
    event.preventDefault();
    {  location.href= '/Home3' ;}};document.getElementById('i2kza').onclick = (event) => {
    event.preventDefault();
    {  location.href= '/HostParty' ;}};document.getElementById('imrach').onclick = (event) => {
    event.preventDefault();
    {  location.href= '/Admin' ;}};const onClickPaginationButton = (chunk, pagination) => {
  ...
  apiPartyApi.getAllparty((error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); const subDataElements =[...document.getElementById("ibgh2").querySelectorAll( "[dataitem='true']" )].filter(
    (element, index, array) =>
  ...
  document.getElementById('i3i04').onclick = (event) => {
    event.preventDefault();
    { 
      ...
      newMap.forEach((value, key) => {
        if (
          document
            .getElementById(key)
            .contains(document.getElementById("i3i04")) === true &&
     ...
     location.href= '/PartyPage/' + transitionId;}};document.getElementById('itxxt').onclick = (event) => {
    event.preventDefault();
    { 
      let transitionId = window.location.href.split('/').at(-1);
      let parentId = "";
      ...
      newMap.forEach((value, key) => {
        if (
          document
            .getElementById(key)
            .contains(document.getElementById("itxxt")) === true &&
            document.getElementById(key).contains(document.getElementById(parentId)) === false
        ) {
          transitionId = value._id;
          parentId = key;
     ...
     location.href= '//' + transitionId;}};document.getElementById('if08p').onclick = (event) => {
    event.preventDefault();
    { 
      let transitionId = window.location.href.split('/').at(-1);
      let parentId = "";
      ...
      newMap.forEach((value, key) => {
        if (
          document
            .getElementById(key)
            .contains(document.getElementById("if08p")) === true &&
            document.getElementById(key).contains(document.getElementById(parentId)) === false
        ) {
          transitionId = value._id;
          parentId = key;
     ...
     location.href= '/PartyPage/' + transitionId;}};window.onload = () => {apiPartyApi.getAllparty((error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); const subDataElements =[...document.getElementById("ibgh2").querySelectorAll( "[dataitem='true']" )].filter(
    (element, index, array) =>
    !array.reduce((hasAncestorFlag, dataItem) => hasAncestorFlag || (element.compareDocumentPosition(dataItem) & Node.DOCUMENT_POSITION_CONTAINS) === 8, false)
